...
export default function SingleCard({ data }: { data: Books }) {
  const [showModal, setShowModal] = useState(false);

  return (
    <div className="border-2 border-gray-500 rounded-lg px-2 m-4 relative hover:shadow-xl">
      ...
      {showModal && (
        <BookModal data={data} onClose={() => setShowModal(false)} />
      )}
    </div>
  );
}
